...
- **Roster sync (Slice 76, ADR 0017)** — `lib/roster-sync.ts`:
  ...
  - **Practice-sitting sweep + asset delete (docs/practice-sitting-design.md, D-7, 2026-09-22)**: the same invocation also runs `sweepPracticeSittings` for overdue practice sittings — it deletes the practice attempts (and their DB rows) through the same path the Delete button uses, and archives the sitting. The importer's own S3 grant now goes further than reading `roster/*`: it also gets **`s3:DeleteObject` on `responses/*` of the design-tool asset bucket, and nothing else on that bucket** (no Put, no Get, no List) — passed in as `assetBucket` on `RosterSync` and exposed to the function as `ASSET_BUCKET`. The delete step itself (`infra/lambda/deleteStoredUpload.ts`) never imports `lib/storage` (that module's S3 provider pulls in `@aws-sdk/s3-request-presigner` for the client-upload flow, which this Lambda does not need and does not bundle); it duplicates just the storage-provider id string and refuses — rather than silently skipping — any ref whose provider isn't `s3` or whose key isn't under `responses/`. Best-effort per upload, logged as `practice_sweep` / `practice_sweep_failed`, same as the D-11 sweep.
