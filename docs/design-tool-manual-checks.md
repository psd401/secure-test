...
| 217 | From the Monitor header on an OPEN sitting, press **Extend time** and pick a time later today, then confirm | Enabled even though the sitting is open (no "End the test session first" gate, unlike Hand in everyone); the extended rows' "Until …" line updates to the picked time without the date (today) | NOT RUN 2026-09-22 — needs an open sitting with an in-progress student in scope (teacher-row script + a join). Observed on the CLOSED `4NH9M8` Monitor: the header's Extend time is rendered in the muted outline style but is not `disabled` (checked in the DOM), consistent with the additive rule |
...
| 234 | After the timed pass back completes, the student rejoins | The countdown reflects the new deadline | NOT RUN — the Row S-f attempt is prepared (in progress, override Sep 23 23:59); needs the student on the client |
| 235 | On the Monitor for an open sitting, find a row with status Submitted (or "Handed in (earlier session)"), press **Pass back** | Same dialog; on success the row updates to In progress / the earlier-session note clears as appropriate | NOT RUN 2026-09-22 — needs an open sitting with the student in scope (teacher-row script) |
