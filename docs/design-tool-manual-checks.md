...
| 240 | Press **All teachers** | The URL gains `?all=1`; the header reads "All teachers' assessments (N)"; the table gains an **Owner** column showing each row's owner email; the link now reads "My assessments" | ✅ 2026-09-21: URL `?all=1`, header "All teachers' assessments (8)", Owner column, link reads "My assessments". Findings A5-1 (Owner "—" on the admin's own pre-0038 rows — owner_email only backfilled from sittings) + A5-2 (the table overflows horizontally in the all view; Delete cut off — same shape as A-1) |
