...
| 6 | **Reporting R3**: gradebook integration — **researched + designed 2026-09-22**: "Send to gradebook" into PowerTeacher Pro (PSD plugin on the `/ws/xte/` API, IT writes it; DCIDs join the extract) or Schoology (three-legged OAuth per teacher, REST v1); one destination per send | `docs/gradebook-integration-research.md` (survey + IT's findings) → `docs/gradebook-push-design.md` (D-1…D-7, slices 0–5, nothing built) | waits on IT: plugin on a test instance + OAuth pair, three extract columns, a Schoology app | design tool; slice 1 Sonnet 5, 2 ∥ 3 Opus 5, 4 Sonnet 5 |
...
Six items James raised on 2026-09-17 (U-7 / U-8 the same evening). None
has a slot in the order; each row records what exists today, the
constraint that shapes it, and the open questions. Answers James gave the
same day are marked **decided**.

**Every roadmap row carries a best-guess "Side"** (James, 2026-09-17): the
order table's Line column, this table's Side column, and any row added
later. The reason is the release path — a design-tool change is live
within the hour of a deploy, while a client change reaches students only
through a release + IT's next-day AutoPkg run, and a client release is a
disruption to every Mac mid-pilot. **Rule for the pilot week: design-tool
work proceeds beside the pilot; no client release before 2026-09-18 14:00
PT at the earliest.** A "both" row is sequenced so the server half ships
first and the client half rides the next release.
...
| Row | Item | Side | What exists / shapes it | Open questions |
|---|---|---|---|---|
| U-1 | **Co-teachers on the same class** | design tool (grants + Share dialog; the client authorizes by sitting + roster) | Staff sharing is copy semantics (2026-09-01); roster sections carry one `teacher_email`; sittings and attempts are owner-only | **Decided: shared ownership is the goal** (one assessment + its sittings, both teachers see the section's attempts) — not a second copy. Roster: YES — `section_teachers.role_name` = `Co-teacher` (measured 2026-09-17, 243 multi-teacher sections); co-teachers already see the section. Design: `docs/access-model-design.md` (D-1…D-8) |
| U-2 | **Substitute visibility + access** | design tool — **DEFERRED 2026-09-18 with U-3 / U-6** (the access-model note's slice 4 + D-5 refinements hold the design: Monitor only, teacher self-delegation ≤ 31 d or admin) | No delegation; a sub's `psd401.net` email gets `staff` but owns nothing | Time-boxed grant by the teacher, or IT / admin-assigned? Monitor-only, or open / close sittings too? Source of "who is the sub today"? |
| U-3 | **Principal visibility + access** | design tool (deferred) | No hierarchy; nothing knows a building | Results only, or live Monitor? Scope = building from the warehouse? Read-only? |
| U-4 | **Teacher self-assign for practice** — **design note written 2026-09-22: `docs/practice-sitting-design.md`** (D-1…D-8 recommended: a `practice` sitting kind naming one staff sub, the student routes admit a practice principal through one resolver branch, invisibility enforced by one test, "Practice on my Mac" + Practice again on the Test sessions tab, lockdown ALWAYS real — the on/off option dropped; slices 1–4, nothing built) | **both** — server M, teacher UI S, client copy XS (rides v1.3.5) | Attempts need a roster student in a section; every student route is `requireStudent`; lockdown is always real in Release; **teacher Macs need the client + profile — the Jamf scope ask went to IT 2026-09-22** | **Decided: in the client app itself** (not a browser preview); **practice attempts excluded from results**. The note's decisions await James |
| U-5 | **Placeholder account for not-yet-enrolled students** | design tool if the placeholder is a real Google account; **both** if not (the client refuses a non-roster student) | Roster is the nightly extract; the `students` overlay is keyed on `roster_ps_id`; nothing creates a student | Placeholder = a real Google account IT provisions, or a staff-created local row merged on enrollment? Who merges, and how are attempts reattached? |
| U-6 | **System admin: see in-progress attempts, impersonate staff** | design tool — **split 2026-09-21 (James)**: the SYSTEM admin (maintainer) is wanted now → access-model slice 5a "All teachers" view (BUILDING in progress); impersonation + grants console (5b) stay deferred with U-2 / U-3 (building admin = principal) | No admin role; audit tables exist (`attempt_deletions`, the error-event tables) | Admin list = a context / env allowlist or a DB table? Impersonation = act-as with an audit row and a visible banner? Read-only view first? |
| U-7 | **Pass back a handed-in attempt** (pilot teachers, 2026-09-17) | **both** — server: the status flip; client: H-1's "already handed in" join refusal must admit a passed-back attempt | `submitted` is terminal: the only exit is delete attempt (loses everything); H-1's join refusal names the earlier session; scores hang off the attempt | **Decided: existing scores are KEPT as a record** — design `docs/pass-back-design.md` (D-1…D-4 decided 2026-09-18: `superseded` score status, `edit`-level route, deadline required when timed, no client change). Open: teacher-only? audit row + event kind (`teacher_pass_back`)? does the student rejoin the same sitting or need a new one? does the timer (U-8) resume or restart? |
| U-8 | **Timer across sittings — working-time clock + teacher extension** (James, 2026-09-17; exploring, not decided). **Extension (b) BUILT 2026-09-17 the same evening** as an absolute override (`attempts.deadline_override_at`, migration 0037, `889cf38` + `621e1d8`; time-limit note §"Teacher: extend time") for the pilot class that timed out on day 1 — the working-time clock (a) stays open; **the live mid-session re-arm (c) BUILT 2026-09-23 as EX-1** (the peek poll carries the deadline, live rev 49; the client re-arm rides v1.3.5) | server for a later-sitting join (built); **client** for a live mid-session re-arm | Today the deadline is wall-clock: `started_at + time_limit_seconds` per attempt, never reset (`lib/api/attemptDeadline.ts`) — a 60-minute test started Monday is expired Tuesday however long the student worked. The client reads `time_limit_ends_at` + `server_now` ONCE from the bundle at join and counts to that instant (`TimeLimitCountdown`); the peek poll carries no deadline | **Decided direction: working-time clock with teacher extension.** How much is server-only: (a) **working time** — the server computes remaining = limit − time spent in earlier sessions (from `lockdown_begin` / `lockdown_end` / `sitting_closed` / `time_expired` events, with the last peek poll as the fallback for a session that never posted its end) and emits `time_limit_ends_at = now + remaining` on the bundle; `refuseIfPastDeadline` uses the same helper — **no client change** for a student who joins a later sitting. (b) **Extension** — an `extension_seconds` column on the attempt folded into the same helper, a Monitor / per-student control — **no client change** for a student who joins AFTER it is granted; a student MID-session still ends at the old zero and continues on rejoin. (c) **Mid-session extension live** needs the peek poll to carry the deadline and the client to re-arm the countdown — a client release (v1.3.5). Open: the clock's granularity (event-based vs heartbeat), whether the Monitor shows working time, and whether U-7 pass-back restarts or resumes it |
| U-9 | **Batch print chosen students and/or sections** (James, 2026-09-23; not mission critical — come back to it) | design tool | "Print student work" prints one section per URL (handed-in only) or, since 2026-09-23, one student via `?attempt=` / the toolbar Student picker (in-progress included) — `docs/student-work-export-design.md` | A checkbox menu: pick several students and/or several sections and print them as one packet. Open: one PDF vs one per section; mixing handed-in and in-progress; anonymous labels across sections (numbering is per section today); ordering (section, then name?) |
...
recommendations, mechanism (Close and expiry END THE SITTING, NOT THE ATTEMPT — the attempt
stays in_progress and resumable in a later session, the server refuses
writes, no grace; the client learns through the 5 s peek poll and the
write 409 and returns to Your tests; Hand in stays the teacher's), three slices
