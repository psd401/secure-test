...
| U-6 | **System admin: see in-progress attempts, impersonate staff** | design tool — **DEFERRED 2026-09-18 with U-2 / U-3**: revisit substitute, admin + principal accounts together (too early to commit to a direction); the access-model note's slice 5 holds the design | No admin role; audit tables exist (`attempt_deletions`, the error-event tables) | Admin list = a context / env allowlist or a DB table? Impersonation = act-as with an audit row and a visible banner? Read-only view first? |
