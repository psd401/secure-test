...
**Row S-f sitting, 2026-09-09 evening (pass 1, REAL AEAssessmentSession, James at the client, Claude on the teacher side in Chrome; fixture `Row S-f hand-run 2026-09-09`, Published on the origin with one handed-in attempt — stays for now).** Client: 26 of 41 rows ✅ (C-2 all seven; C-1 two columns in the real session with the log order DID BEGIN → `page load gate: opened` → build, no backstop line, toggle group incl. keyboard / page turn / second set / essay text; C-4 pinch, ⌘= / ⌘- / ⌘0 with the log's 1.25× steps, overlay open / Esc / backdrop / picture / stem caption, hotspot untouched). The pinch-past-3× row ✅ by James's eye (it snapped back) with NO `zoom: pinch clamped` line in the log: WebKit's own pinch bounds rubber-band to 3× before our KVO clamp ever sees an out-of-range value — the KVO path is belt-and-braces. Not run: the simulated pass, contrast sets, narrow / `refuses` / `slow`, the 1.5X accommodation, entry-screen menu state, Enter / Space on a picture, Tab-out, dialog swap, VoiceOver. Teacher: rows 93–106 + 108 ✅, 107 (forced failure) not run; **row 96 proved the prompt change on Bedrock** — every amount came back as `\$`. Cosmetic finding **S-f-1**: the PDF panel's proposal cards print the raw `\$` (proposal only). **S-f-1 FIXED 2026-09-09** — the card's one-line preview drops the escape (`cardText`); the stored stem keeps `\$`. Scratch draft `C-6 autosave scratch 2026-09-09` deleted from the origin the same evening (there is no delete control in the editor or the list — `DELETE /api/assessments/<id>` from the signed-in page, 204).
