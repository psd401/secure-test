...
- **Rows 109–119 RUN 2026-09-10** (Claude in Chrome on the origin as James, rev 18): 109–112, 114–119 ✅; 113 blocked (no roster section that day → no open session; the expired-open sitting archived fine, as designed). Fixtures: two scratch drafts created and deleted; the accommodations fixture archived and unarchived (its closed sitting too), left as before. **Findings, unbuilt:** **A-1** the Assessments list's actions cell (Results + Archive + Delete) overflows the card at a 1470 px viewport, so Delete hides behind a horizontal scroll — shorten the Questions column to the bare number or move Archive / Delete into a row menu; **A-2** Unarchive from the archived view reloads `?archived=1` and lands on the empty archived page — navigate to `/dashboard` instead. Clock note: `archived_at` matched the browser's `Date.now()` exactly; server and browser agree.
- **A-1 + A-2 BUILT 2026-09-10** (James: shorter Questions column): the list's Questions cell shows the bare number (the header names it); `ArchiveAssessmentButton` navigates to `/dashboard` after an unarchive and reloads after an archive. Rides the next deploy; re-check rows 110 (Delete visible without a horizontal scroll at 1470 px) and 116 (lands on the live list).
- **DEPLOYED + RE-CHECKED 2026-09-10 midday**: A-2 ✅ on rev 19 (`100da2a`); A-1 took three cuts — the bare number (19 px), `truncate` on the link under `max-w-md` (no change: auto table layout ignores a cell's max-width), then `w-full max-w-0` on the name cell (`3e35d36`, rev 21) — ✅ on the origin: 846 px table in the 846 px card, Delete inside, long names ellipsised. LIVE = rev 21, `/api/health` commit `3e35d36`.
