...
- 2026-09-18 — **slice 4 BUILT** (substitutes; migration **0039**,
  `0039_sitting_created_by_email.sql`, applied to dev + test). What landed:
  - **`POST /api/coverage`** — a teacher arranging their own cover.
    Self-delegation ONLY: `scope_kind` is always `teacher`, `scope_id` always
    the CALLER's own email taken from the session (never the body), `level`
    always `run`, and `ends_at` is REQUIRED, after `starts_at`, and at most
    `COVERAGE_MAX_DAYS` = 31 days after it (`validateCoverageRange`, measured
    from the start so back-dating cannot stretch the cap) → 400
    `range_invalid` / `range_too_long`. Grantee rules are
    `validateGrantRequest`'s, shared with the Co-teach surface (staff only,
    never self); the live partial unique index gives 409 `already_granted`.
    `GET /api/coverage` answers `{ granted, covering }` — what the caller has
    given (including revoked rows, which the card marks rather than hides) and
    the LIVE grants they hold. `DELETE /api/coverage/[grantId]` revokes for the
    granter or an admin and 404s for everyone else **including the grantee** (a
    sub cannot hand the class back), and refuses a non-`teacher` scope so the
    Share dialog's rows are not reachable through it. Everything is written
    through `createGrant` / `revokeGrant`, so coverage is an ordinary
    `access_grants` row with no second resolution path; `createGrant` gained an
    optional `starts_at` for it. An admin's uncapped grant stays `/api/grants`.
  - **The sub's sections** — `lib/roster/coverage.ts`:
    `coveredTeachersFor(db, session)` (live `teacher`-scope grants at `run` or
    above — a `view`-level teacher grant, D-7's deferred principal, confers no
    sections) and `coveredSectionsFor(db, session)` =
    `sectionsCurrentlyTaughtBy` per covered teacher, each section tagged
    `covering_for`. `GET /api/roster/sections` appends them to the caller's own
    with `covering_for: null` on their own rows, drops a section they both teach
    and cover (their own is the stronger claim), and reports `student_count: 0`
    on a borrowed one rather than running a second count query per covered
    teacher for a number that is decoration on a menu entry. The picker labels
    it "· covering for \<teacher\>".
  - **Sitting creation** — slice 2's branch is now complete. It already chose
    the OWNER for a `teacher`-scoped grant; what it still validated against the
    CALLER's roster was the scope, which refused every section a sub was there
    to run. The roster side is now the covered teacher's (`rosterEmail`), so
    `section_ps_id` must be one of the ASSESSMENT owner's current sections and
    `student_ps_ids` must be their students. A `teacher`-scoped grant resolves
    only through `assessments.owner_email`, so "covering" already means the
    caller holds live cover of exactly that address — the same set
    `coveredSectionsFor` offered. Admission is unchanged (the sitting's own
    `owner_email` + section), so the teacher's students join a sub-created
    sitting exactly as they would have; there is a test that walks that.
  - **Monitor only (D-5 refined)** — `results: boolean` rides on every `ok:
    true` access result, decided by `scopeConfersResults(scope)` in grants.ts:
    false for a `teacher` scope, true for an owner, an admin and an
    `assessment`-scoped (co-teacher) grant. It is a property of the SCOPE, not a
    level and not a per-route flag list, because a sub genuinely needs `run`.
    Two doors enforce it: `refuseResults(access)` (the 404) in
    `GET …/results` (incl. `?format=csv`), `…/review-queue`, `…/export`,
    `GET /api/attempts/[id]/events`, `GET /api/responses/[id]/upload` (the saved
    drawing), `POST /api/attempts/[id]/score` and `score-ai`, and
    `loadResponseChain` (so `responses/*/score`, `rescore-ai` and
    `scores/*/approve` refuse explicitly even though their `edit` level already
    excluded a sub); and `pageResultsAssessment(db, session, id, need = "view")`
    replacing `pageAssessment` on the results matrix, the per-student page, the
    print report, the work packet and the scoring queue.
  - **The sub's editor** — `app/dashboard/[id]/page.tsx` authorizes at **`run`**
    rather than `edit` (the Test sessions tab is where a sitting is started) and
    does NOT fetch items or item sets for a caller below `edit`: a substitute has
    no business holding the answer keys, even in a tab that never renders. Pure
    `tabsForAccess(access)` returns `["sessions"]` below `edit` and all five
    otherwise — not a disabled-tabs view, since a greyed-out Questions tab would
    advertise what it hides and every route behind the other four is `edit`
    anyway. A requested `?tab=` the caller does not have falls back to
    `tabs[0]`. Gated off for them: Share, Publish/Unpublish, the publish-lock
    notice, and the whole Download backup / Scoring queue / Results line (all
    three are `results` routes). `coveringBadgeText` gives them "Covering for
    \<teacher\>"; `coTeachingBadgeText` now returns null below `edit`, so the two
    badges are mutually exclusive.
  - **Home page** — a **Coverage** card below the Open-now strip
    (`app/dashboard/CoverageCard.tsx`): a banner per live grant held, the
    caller's own outgoing grants with dates, note and Revoke, and the add form
    (staff email, first day, last day defaulting to today, note). Both lists are
    loaded by the SERVER page, not by an effect — the row labels need the same
    facts, and the dashboard tree renders headless in
    `test/reporting-views.test.tsx` where no effect runs; writes reload the page
    rather than using `useRouter`, the existing list-row-leaf pattern. The card
    is hidden entirely when there is no coverage either way. A `level === "run"`
    grant row now reads "Covering for \<teacher\> · until \<date\>" instead of
    slice 3's co-teacher label, links to `?tab=sessions`, and drops its Results
    button.
  - **Sitting provenance** — migration 0039 adds
    `test_sessions.created_by_email` (nullable, audit only), written at creation
    beside `created_by_sub`; pure `coveringStartedBy(sitting)` in
    `attendanceView.ts` prints "Started by \<email\> (covering)" when
    `created_by_sub` differs from `owner_sub`, on the Test sessions row and in
    the Monitor header (the monitor page computes it server-side rather than
    shipping three columns the client has no other use for).

  **Deviations from the brief, both deliberate:**
  1. **An uncovered section is 400 `section_not_taught`, not 404.** The brief
     said 404. A section ps_id is not a row the access helper protects — the
     assessment is already authorized by then, the picker only ever offered
     covered sections, and the route has answered 400 `section_not_taught` for
     the owner's own bad section since slice 79, with teacher copy for that code
     already in `lib/ui/errorCopy.ts`. Making the same mistake 404 for a sub and
     400 for a teacher would be two answers to one question and would lose the
     copy. The 404-for-access rule (D-3) is untouched: an assessment the caller
     does not cover is still 404, tested.
  2. **The Monitor has no Results links to hide.** The brief asked for the
     per-row "Results" links and the header's Results button to hide when
     `results === false`; `MonitorView.tsx` has never had either (its only links
     are "Back to test sessions"). The results links that DO exist — the editor
     header line and the home list's row button — are gated as described above.

  Tests: `test/coverage-api.test.ts` (the three routes: self-scoping, the cap,
  the staff / self / duplicate refusals, re-granting after a revoke, live-only
  `covering`, revoke by granter / admin / grantee / stranger, the pure copy),
  `test/coverage-sections.test.ts` (`coveredTeachersFor` /
  `coveredSectionsFor` incl. the `view`-level and expired cases, the sections
  route's tagging, a sub creating a sitting on a covered section with the
  teacher as owner, an uncovered section refused, no grant → 404, and **the
  teacher's student joining the sub-created sitting**),
  `test/coverage-results-refusal.test.ts` (every results surface 404 for a
  teacher-scope grantee while attendance / peek / extend / hand-in-all / close
  all work; a co-teacher by contrast reads results), `test/coverage-ui.test.tsx`
  (`tabsForAccess`, the two badges' exclusivity, `coverageErrorCopy`, and the
  Coverage card's static markup), plus `coveringStartedBy` in
  `test/attendance-view.test.ts` and the two new routes classified in
  `test/access-enforcement.test.ts` (`NO_OWNED_ROW`: they address a grant, not
  an owned row). `test/auth-role-enforcement.test.ts` discovers routes from
  disk, so both are swept by it with no registration. Design-tool **2083** tests
  (2040 before), typecheck clean. Rows **231–249** in
  `docs/design-tool-manual-checks.md` ("Substitutes — Coverage") are NOT RUN —
  most need a SECOND staff account, one needs a student sitting, and the sub's
  section list needs a current roster day for the covered teacher.
