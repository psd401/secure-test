...
public enum AssessmentPage {
    ...
    .order-row {
      ...
      border: 1px solid #e5e5ea; border-radius: 6px; background: #fff; cursor: grab;
      -webkit-user-drag: element;
    }
    .order-row:active { cursor: grabbing; }
    .order-position { min-width: 1.6em; color: #3a4a6a; }
    .order-label { flex: 1; line-height: 1.4; }
    .order-move { font: inherit; padding: 2px 8px; border: 1px solid #c7c7cc; border-radius: 6px; background: #fff; color: #1c1c1e; cursor: pointer; }
    .order-move:disabled { opacity: .4; cursor: default; }
    .order-row.dragging { opacity: .5; }
    .order-row.drop-before { box-shadow: inset 0 0.125rem 0 0 var(--accent, #0b5cd6); }
    .order-row.drop-after { box-shadow: inset 0 -0.125rem 0 0 var(--accent, #0b5cd6); }
    .order-hint { margin: 0 0 6px; font-size: 13px; color: #3a4a6a; }
    .order-status { margin: 4px 0 0; font-size: 12px; color: #555; }
