...
| Install the configuration profile (`client/config-profile.example.mobileconfig`, real values from the ops repo) FIRST, then install `SecureTest-1.0.0.pkg` from the release on a district Mac (Jamf or by hand), launch `/Applications/SecureTest.app` | Gatekeeper opens it with no warning; the card shows Sign in with Google (NOT the not-set-up message) and stderr reads `config: server URL from managed preference` / `config: google client id from managed preference`; sign in, join, one real session (`REAL AEAssessmentSession` → `DID BEGIN` → hand in → `DID END`), no TCC prompt; About reads `1.0.0 (c719eb586e45)` | **✅ 2026-09-14 — installed through Jamf** (v1.3.0: the pkg by policy plus the `com.apple.ManagedClient.preferences` profile; IT's install went without incident) on a district student Mac, James at the keyboard, Claude on the teacher side: a Finder launch showed Sign in with Google (the managed preferences reached the sandboxed app — NOT the not-set-up message), no Gatekeeper warning reported; a real student signed in, "Your tests" listed the section sitting, join → the Mac locked (real session), the pilot assessment's four sources rendered, a teacher peek notice arrived and was dismissed, hand in → the Mac came back and Your tests showed the test as handed in; no TCC prompt at any point. A second real session the same afternoon ran to the time limit and ended on its own. Not read: stderr (Finder launch) and About's version string. **Batch 2's last row — closed** |
...
| Check | Expect | Result |
|---|---|---|
| Launch the app and look at stderr | `[security] errors.log open at …/SecureTest/errors.log` — the sink opened. (If it says "errors.log unavailable", every row below is moot and that is the finding) | ✅ 2026-09-08 (client sitting, real AAC session, Debug build of `a755671`): the sink opened at launch and stayed empty through the whole sitting — nothing failed |
...
| Launch with `SECURE_TEST_DEBUG_CRASH=1`, sign in, join a sitting, then Session → Trigger Debug Crash (SIGABRT) | The app dies at once (macOS may show its own crash report — that is the OS, not us). `errors.log` gained ONE line: `"kind":"crash"`, `"message":"fatal signal SIGABRT"`, this build's version and commit, and the `attempt_id` of the sitting just joined. It has NO `occurred_at` — a signal handler cannot read a clock | ✅ 2026-09-08 (sitting pass two; the launcher forwards `SECURE_TEST_DEBUG_CRASH`, `528833f`): the app died at once; `errors.log` held exactly one line — `{"kind":"crash","message":"fatal signal SIGABRT","app_version":"1.0.0","app_commit":"5b65f4cc28ca"}`, no `occurred_at` |
| Relaunch and sign in after that crash | The drain sends the crash line; the server row carries `occurred_at` stamped at drain time (documented, not a bug) and `context.attempt_id` naming the attempt that died | ✅ / ⚠️ half 2026-09-08: the next launch's sign-in logged `client-error drain sent 1 of 1 line(s)` and the file was empty afterwards; the server-side `client_error_events` row (its `occurred_at` and `context.attempt_id`) was not read back |
...
| Check | Expect | Result |
|---|---|---|
| Join the sitting, reach the side-by-side question on the fullscreen client | One page: the introduction and the source tabs on the left (about 55 % of the width), the essay box on the right; the page does not scroll as a whole | ⚠️ split: ❌ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 16, fixture `Multi-source hand-run 2026-09-09`) — the page BUILT NARROW inside the real session and took the own_page fallback (a passage page, then the question with the collapsed pane); ✅ 2026-09-09 pass 2 (simulated lockdown, fixture `… (accommodations)`) — sources left, essay right on one page. Finding **C-1** (`docs/multi-source-stimulus-design.md`): the AAC begin() transition resizes the window at build time |
| Click "Source B", then "Source A" | The panel under the strip swaps; exactly one source is visible at a time; the selected tab carries the accent underline and fill | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 16, fixture `Multi-source hand-run 2026-09-09`): tabs clearly identified, one panel at a time |
| Scroll inside a long source panel | The panel scrolls on its own; the essay box on the right stays where it is | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 16, fixture `Multi-source hand-run 2026-09-09`) |
| Tab into the tab strip, then press Right, Right | The strip is ONE Tab stop; Right moves the selection AND the focus and wraps from the last tab back to the first | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 16, fixture `Multi-source hand-run 2026-09-09`) (James: "tab and home are good"; the Right-wrap was not called out separately) |
| Press Left, then Home, then End | Left wraps backwards; Home opens the first source; End opens the last | ⚠️ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 16, fixture `Multi-source hand-run 2026-09-09`): Home ✅; Left and End not called out |
| Tab once more from the selected tab | Focus lands on the open panel (it is focusable so the source can be scrolled by keyboard), then on the essay box | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 16, fixture `Multi-source hand-run 2026-09-09`) (keyboard pass reported good; the panel focus stop not called out separately) |
| VoiceOver (Cmd-F5) on a tab | Announces the label, "tab", selected state and the position in the strip ("Source A, selected, tab, 1 of 2") | ❌→not run 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 16, fixture `Multi-source hand-run 2026-09-09`): Cmd-F5 opened VoiceOver's first-run Quick Start, which is another app and is invisible / unresponsive under AAC (finding **C-3**). The tutorial was dismissed outside the session afterwards; the VoiceOver rows were NOT re-run — next sitting |
| VoiceOver on the open panel | Announces the panel as labelled by its tab, then reads the source text; an image in Source B is read by its alt text | Not run (see the row above) |
| Look at Source A's text | The authored blank line is still a line break — the paragraphs did not collapse into one run of prose; the `**bold**` heading is bold | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 16, fixture `Multi-source hand-run 2026-09-09`) |
| Look at Source B | The image renders at a sensible size inside the panel, not overflowing it | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 16, fixture `Multi-source hand-run 2026-09-09`) — on this fixture the images are the three charts in Source C: rendered at a sensible size under their titles. James asks for student-side chart zoom → finding **C-4** |
| The set with exactly one source | No tab strip at all — a heading with the source's label above its text | Not exercised — the fixture has one set with three sources |
| Type an answer in the essay box, then click a tab and come back | The typed text is untouched; switching sources never touches the answer | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 16, fixture `Multi-source hand-run 2026-09-09`) |
| Move to the next question and back with the pager | The source that was open is still the one open (kept in memory, nothing posts) | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 16, fixture `Multi-source hand-run 2026-09-09`): the open source persisted |
| Accommodations: Zoom 150 % on the side-by-side page | Both columns stay usable; if the window is now effectively narrow the columns stack rather than crushing — record which happens (the note's open question: is 1100 px the right threshold at 150 %?) | ✅ 2026-09-09 pass 2 (simulated lockdown, fixture `… (accommodations)`) with `zoom = 1.5X` on the demo student's override for the copy; stack-vs-side-by-side at 1.5X not recorded. **Setup finding C-5:** there is NO student toolbar — the three accommodations paint from the VALUE in the student's row (TIDE strings: `1.5X`, `Reverse Contrast`, `On`), gated by the assessment's allowed list, and overrides are PER ASSESSMENT (the first attempt set them on the original while the student sat the copy). Pass 1 had none set, so nothing showed |
| Accommodations: Color Contrast = Reverse Contrast (and one more set, e.g. Yellow on Blue) | Tab labels, the selected tab's fill and the source text are all readable; the selected tab is distinguishable by more than colour (it also carries the underline) | ✅ 2026-09-09 pass 2 (simulated lockdown, fixture `… (accommodations)`): Reverse Contrast readable on tabs, fill and text; Yellow on Blue not run separately |
| Optional font = Atkinson Hyperlegible | The source text and the tab labels both take the font | ✅ 2026-09-09 pass 2 (simulated lockdown, fixture `… (accommodations)`) |
| **Narrow fallback:** relaunch with `SECURE_TEST_NO_FULLSCREEN=1` and drag the window under about 1100 px, then join and reach the set | The set is a passage page ("Passage for question N") carrying the sources, then the question page with a collapsed disclosure reading **"Show the sources"** (not "Show the passage") | ✅ by accident in 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 16, fixture `Multi-source hand-run 2026-09-09`): the real session built narrow (C-1), so the student saw the passage page and then the question with the collapsed pane — James: "having them on both pages was good". The disclosure's exact label was not read out. The launcher does NOT forward `SECURE_TEST_NO_FULLSCREEN` (finding **C-7**), so pass 2 stayed fullscreen |
| On that question page, open the disclosure | The same source pane appears in place — tabs still work, and there is only ever one copy of the pane on the page | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 16, fixture `Multi-source hand-run 2026-09-09`): the pane worked in place; single-copy not verified |
| A set with no sources, narrow | Its disclosure still reads "Show the passage" | Not exercised — no such set on the fixture |
| Drag the window narrower AFTER the page has built (side-by-side already on screen) | The two columns stack instead of crushing; the page does not re-page (the layout is chosen once at build — this is the CSS safety net, deliberate) | ✅ 2026-09-09 pass 2 (simulated lockdown, fixture `… (accommodations)`): sizing down moved the sources above the question; widening put them back on the left (the CSS media query, no re-page) |
...
| Check | Expect | Result |
|---|---|---|
| Open Source A | `Costs rose from $57,600 to between $30,000–$120,000 a year.` renders with every `$` literal — none of the four dollar signs opens math | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 17, client from 09a0b65, fixture `Row S-f hand-run 2026-09-09`) — James's step-2 pass (money literal, `$5`, both math runs, `$5x$` literal, display block) |
| In the same source, find `\$5` | Renders as the literal text `$5` | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 17, client from 09a0b65, fixture `Row S-f hand-run 2026-09-09`) — James's step-2 pass (money literal, `$5`, both math runs, `$5x$` literal, display block) |
| In the same source, find `$x^2 + 1$` | Renders as math: x², properly spaced | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 17, client from 09a0b65, fixture `Row S-f hand-run 2026-09-09`) — James's step-2 pass (money literal, `$5`, both math runs, `$5x$` literal, display block) |
| In the same source, find `${5x+3}$` | Renders as math (the brace escape hatch for a digit-start expression) | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 17, client from 09a0b65, fixture `Row S-f hand-run 2026-09-09`) — James's step-2 pass (money literal, `$5`, both math runs, `$5x$` literal, display block) |
| Open the essay stem | `$5x$` renders as literal text `$5x$`, not math | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 17, client from 09a0b65, fixture `Row S-f hand-run 2026-09-09`) — James's step-2 pass (money literal, `$5`, both math runs, `$5x$` literal, display block) |
| Cross-reference: compare Source A's rendering here against the design tool's preview of the same set (row 93) | Both surfaces agree on which runs are money and which are math | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 17, client from 09a0b65, fixture `Row S-f hand-run 2026-09-09`): the design tool's preview and print of the same set (rows 93–94, Claude in Chrome) show the same split — every `$` literal, `$5`, `x² + 1`, `5x + 3`, `y = 2x + 1` display |
| Look for a `$$…$$` display block anywhere on the fixture (add one to Source A if none was authored) | Unchanged — still renders as a display equation | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 17, client from 09a0b65, fixture `Row S-f hand-run 2026-09-09`) — James's step-2 pass (money literal, `$5`, both math runs, `$5x$` literal, display block) |
| **Real session:** join, reach the side-by-side set | Two columns (source pane left, essay right), not the own_page passage-page fallback; before the page appears the student log shows `page load gate: opened` | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 17, client from 09a0b65, fixture `Row S-f hand-run 2026-09-09`): two columns on the first page (James); student log order `lockdown session: REAL AEAssessmentSession` → `lockdown DID BEGIN` → `page load gate: opened` → the build |
| **Simulated session:** same set | Same two-column layout; log shows `page load gate: opened` too | |
| Across both passes, grep the student log for the backstop line | `page load gate: backstop after 5 s — building anyway` never appears in a normal run (the gate should open promptly, not time out) | ✅ 2026-09-09 pass 1: no backstop line in the log (real session only — the simulated pass is not yet run) |
| Look at the two-column page | A `role="group"` toggle reads "Sources: Beside the question \| Above the question" sits above the split | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 17, client from 09a0b65, fixture `Row S-f hand-run 2026-09-09`) (step 3) |
| Click "Above the question", then "Beside the question" | The source pane moves above the essay (the split gains `stacked`), then returns beside it | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 17, client from 09a0b65, fixture `Row S-f hand-run 2026-09-09`) (step 3) |
...
| Tab to the toggle group, then Tab again | Both buttons are reachable by Tab; Space or Enter on a focused button activates it | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 17, client from 09a0b65, fixture `Row S-f hand-run 2026-09-09`) (step 3: Tab reached both, Space activated) |
| Set the toggle to "Above", turn to the next question with the pager, then back to the set's page | The "Above" choice is still in effect (in-memory, per set) | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 17, client from 09a0b65, fixture `Row S-f hand-run 2026-09-09`) (step 6) |
...
| Press ⌘- from 3× | Steps back out ×1.25 per press toward 1× | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 17, client from 09a0b65, fixture `Row S-f hand-run 2026-09-09`): log shows ⌘- stepping 1.56x → 1.25x → 1.00x (from 1.56×, not from 3×) |
| Press ⌘0 | Returns to Actual Size (1×) from any zoom level | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 17, client from 09a0b65, fixture `Row S-f hand-run 2026-09-09`): log `zoom: 1.00x` after ⌘0 from 2.44x |
...
| Click a picture inside a source | An overlay opens: the picture large, a caption reading "<source label> — <alt>", a Close button that is focused | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 17, client from 09a0b65, fixture `Row S-f hand-run 2026-09-09`) (step 5: caption "B — chart", Close focused) |
| Click a picture in the essay stem (not inside a source) | Overlay opens with the alt text alone as the caption (no source-label prefix) | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 17, client from 09a0b65, fixture `Row S-f hand-run 2026-09-09`) (step 5) |
| Press Esc while the overlay is open | Closes; focus returns to the picture that opened it (visible focus ring) | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 17, client from 09a0b65, fixture `Row S-f hand-run 2026-09-09`) (step 5: focus ring back on the chart) |
| Click the dark backdrop area of the overlay | Closes the overlay | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 17, client from 09a0b65, fixture `Row S-f hand-run 2026-09-09`) (step 5) |
| Click the enlarged picture itself inside the overlay | Also closes the overlay | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 17, client from 09a0b65, fixture `Row S-f hand-run 2026-09-09`) (step 5) |
| Press Enter, then separately Space, on a focused (not-yet-open) picture | Both open the overlay | |
| Click the hotspot item's picture | It does NOT enlarge — no overlay opens, and the hotspot regions still respond to clicks for answering | ✅ 2026-09-09 pass 1 (REAL AEAssessmentSession, fullscreen, origin rev 17, client from 09a0b65, fixture `Row S-f hand-run 2026-09-09`) (step 6: answered, no overlay) |
...
| Check | Expect | Result |
|---|---|---|
| **Simulated.** Join the limited fixture, look under the header | A "Time left 3:00" banner appears immediately (first tick fires on `start()`, never a blank minute) | ✅ 2026-09-14 in a REAL session (v1.3.0 from Jamf on a district student Mac, a real student, James's report; the log not read — Finder launch): the banner was there from the start and counted from 3:00 |
| **Simulated.** Join the no-limit fixture | No banner anywhere on the page | |
| **Simulated.** Watch the banner for several seconds | Counts down one second at a time (2:59, 2:58, …) | ✅ 2026-09-14 (real session): watched down to 2:11 |
| **Simulated.** At the very first tick on the 3-minute fixture | The "5 minutes left" amber notice already shows (late-start case: 3:00 remaining is already under the 5-minute threshold) | ✅ 2026-09-14 (real session): the 5-minute notice appeared as the test was loading, then the 3-minute countdown ran |
...
| **Simulated.** Click the banner's × | Banner disappears immediately; its accessible label is "Hide the timer" | ❌ 2026-09-14 (real session) — **finding T-1**: the strip did NOT disappear; it froze at 2:11 (the host stopped pushing text, the element stayed on screen). Cause: `.time-limit { display: flex }` outranks the UA stylesheet's `[hidden] { display: none }`, and the page has no `.time-limit[hidden]` rule (every other hideable on the page has one). Fix = that one rule + a test; ships in v1.3.1. **BUILT 2026-09-14** (`.time-limit[hidden] { display: none; }` + `testTheStylesheetHidesTheStripOnceHidden`) — re-run on v1.3.1 |
| **Simulated.** After hiding the banner (previous row), wait for the 1-minute notice | The notice still appears — hiding the banner does not hide the notices (D-3: they are not the student's to switch off) | ⚠️ 2026-09-14 (real session) — **not observed (finding T-3)**: after the × at 2:11 no 1-minute notice was seen before the session ended at zero. Unverified whether it fired — the notice is host-driven (`clock.onNotice` → `showTimeNotice`) and independent of the strip, but the log was not readable on a Finder launch. Re-check with a Terminal launch (`time limit: 1 minute left` in stderr) once T-1 is fixed |
...
| (b) `defaults write net.psd401.securetest.client ServerURL https://<origin>` and `… GoogleClientID <native id>`, relaunch from Finder | No message; Sign in with Google is back. stderr: `config: server URL from managed preference` and `config: google client id from managed preference`. Sign in, see Your tests, join a sitting — all as normal | ✅ in substance 2026-09-14 through the REAL Jamf profile (an MDM `Forced` payload, not `defaults write`): a Finder launch with no arguments and no environment showed Sign in with Google; sign in, Your tests, join, a real session — all as normal. The stderr half and the `defaults write` form NOT run |
...
| Check | Expect | Result |
|---|---|---|
| Join a sitting, answer an item, press **Cmd-E** | The session ends and the app lands on "Your tests" by itself — no test content anywhere on screen at any point — then a one-button sheet: "Secure session ended" / "Your answers are saved. You can rejoin from Your tests." / **OK**. No "Stay here". stderr: `emergency end control pressed`, `DID END`, `secure session ended without a hand-in — leaving the test screen` | ✅ 2026-09-22 — Debug build from `5b5bf41` (1.3.5), simulated lockdown, origin rev 48, practice sitting on `Math entry + drawing tools hand-run 2026-09-22`, Claude driving the client (computer use) + Chrome: typed into the essay, Cmd-E → home first, then the one-button "Secure session ended" sheet, no "Stay here"; stderr `emergency end control pressed` → `DID END` → `leaving the test screen` |
| Same again with the titlebar **End secure session** button | Identical to the Cmd-E row | ✅ 2026-09-22 — 1.3.5 Debug, simulated lockdown: identical landing and stderr |
| Press **Escape** on that sheet instead of clicking OK | Nothing else happens (the sheet stays until Return / OK — F-B) — the student is still on "Your tests", no test is re-rendered, no second sheet | ❌ 2026-09-22 — 1.3.5 Debug, simulated lockdown: Escape does nothing — the sheet stays up; Return / OK dismiss it. **F-B**: NSAlert binds Escape only to a button titled "Cancel". ACCEPTED as-is (James, 2026-09-22) — the `presentOnEntryWindow` comment corrected; nothing else happens on Escape, which is the part that matters |
| Rejoin the same sitting after an emergency end | The attempt resumes with every earlier answer prefilled (P-1 restore) — nothing spooled was lost by the trip home | ✅ 2026-09-22 — 1.3.5 Debug, simulated lockdown: Resume restored the short-text answer and the essay |
| Join with `SECURE_TEST_SIMULATE_LOCKDOWN=refuses` | **No test is rendered at any point.** The app returns to "Your tests" with a one-button sheet: "Couldn't start a secure session" / "Your test didn't open. Ask your teacher for help." / OK. stderr: `lockdown FAILED TO BEGIN — simulated refusal`, `page load gate: REFUSED …`, `SECURE START REFUSED (session_refused) …`, and a `secure_start_refused` line in `errors.log` | ✅ after the F-A fix — 2026-09-22 — 1.3.5 Debug, simulated lockdown: no test rendered; stderr `FAILED TO BEGIN — simulated refusal`, `page load gate: REFUSED`, `SECURE START REFUSED (session_refused)`, `secure_start_refused` drained to the server. **F-A** (first run, `5b5bf41`): after OK a SECOND sheet "Secure session ended" appeared — the `.idle` return-home path had passed its guard and was waiting on the async input flush when `couldNotStartSecurely` went home; its late `goHome` stacked the second sheet. Fixed: `goHome` returns unless the attempt screen is still up; re-run on the fixed build showed one sheet and one `showEntry` |
| Join with `SECURE_TEST_SIMULATE_LOCKDOWN=interrupts` | Same as the `=refuses` row if the interruption lands before the page built; if it lands after, the ordinary "Secure session ended" landing. Record which happened | ✅ (same landing as `=refuses`) 2026-09-22 — 1.3.5 Debug, simulated lockdown: `DID BEGIN` then `INTERRUPTED` before the page built → the refusal path, same stderr. Showed the F-A double sheet before the fix (same cause, same fix) |
| Join with `SECURE_TEST_SIMULATE_LOCKDOWN=slow` (begin lands after 2 s) | The test DOES open, about 2 s after the join, with `page load gate: opened` in stderr and no refusal or timeout line — the simulated path the launcher and every rehearsal depend on still works | ✅ 2026-09-22 — 1.3.5 Debug, simulated lockdown: `begin() called` then `DID BEGIN` then `page load gate: opened`; no refusal / timeout line |
...
| Check | Expect | Result |
|---|---|---|
| **Close ends the student's session (real session).** Student joined and typing in an essay; teacher presses Close session on the Monitor | Within ~5 s the Mac unlocks, the test comes off the screen, "Your tests" is back, and a one-button sheet reads **"Your teacher ended the test session."** / "Your answers are saved." stderr: `sitting closed (peek) — ending the secure session`, then `DID END` | ✅ 2026-09-16 **REAL session on the released v1.3.3 (arrived through Jamf / AutoPkg on the student device, Finder launch, About = 1.3.3 (f2cc2e57ea4d))**: Close 19:46:46.06Z → `sitting_closed {via: peek}` 19:46:46.48Z → `lockdown_end` 19:46:49.56Z (real AAC teardown); sheet copy exact. (First pass 2026-09-15 on the Debug build under simulated lockdown: 0.4 s to the event as well.) |
| **The event lands on the timeline.** Same student, on the teacher's per-student results page afterwards | A `sitting_closed` event at that moment (detail `via: peek`), and the attempt still reads **In progress** — no hand-in | ✅ 2026-09-15 — `sitting_closed` `via: peek`, "Session closed by the teacher — returned to Your tests" on the per-student page; attempt In progress, Hand in enabled |
| **A student mid-typing loses at most the unflushed field (D-3).** Student typing in an essay when Close lands; compare what the teacher's queue shows | Everything posted before Close is there; at most the field still holding focus is short of its last keystrokes — an autosave in flight at the moment of Close can be refused. No error dialog about lost answers | ✅ 2026-09-15 / 2026-09-16 — essay "Here I am writing." (posted on leaving the field earlier) intact on the teacher page; the words typed in the still-focused essay right before the real-session Close were NOT saved (refused, D-3 as designed); no error dialog; no Needs-attention from the close |
| **A refused write ends it too (real session).** Close the session, then have the student answer one more item BEFORE the next poll lands (within 5 s) | The same landing and the same sheet, reached through the write instead: stderr `responses DROPPED because the sitting is closed — expected, not reported`, `sitting closed (write)`, event detail `via: write`. **No** `responses_dropped` error row and nothing lights "Needs attention" | NOT RUN (the poll won both times within 0.5 s) |
| **An expired session ends the same way.** Start a sitting whose "How long" runs out while the student is working (a short session), do not press Close | At expiry + one poll the student lands home with the same sheet and a `sitting_closed` event. Nothing fires at the exact instant of expiry — it is lazy (D-2) | ✅ 2026-09-15 — 2-minute sitting `YEA769` created 23:24:05Z; `sitting_closed {via: peek}` 23:26:02.5Z then `lockdown_end`; same sheet; nothing fired at the instant, the next poll did |
| **Resume comes back only with an open session.** After the sheet, look at "Your tests"; then have the teacher open a NEW session for the same section and look again | First: the test is not offerable (no Resume). After the new session: the test is listed with **Resume**, and resuming shows every earlier answer prefilled | ✅ 2026-09-15 — after the sheet "No tests assigned right now"; new session `XSXUS7` → Resume, answers and essay restored |
| **One sheet, not two.** Close the session on a student whose time limit is also nearly up; or press Close twice | Exactly one sheet, whichever end arrived first, and exactly one attempt event for it. A second Close is a no-op on the client | ✅ 2026-09-15 (second Close half) — a second Close via the route returned 200 `in_progress: 1`; one `sitting_closed` per close on the timeline. Time-limit race half NOT RUN |
| **Hand in is still the teacher's.** After the student is home, teacher presses Hand in on the Monitor row | The attempt finalises with the answers as they stood; the student's screen is unaffected (they are already on "Your tests") | ✅ 2026-09-16 — Hand in from the per-student page after the real-session close: "Handed in by teacher 12:47 PM", essay exactly as it stood, auto-score ran (2 / 13, 1 unscored); the student was already on Your tests |
...
| Check | Expect | Result |
|---|---|---|
| **Idle autosave lands without a blur.** Join, click into an essay, type a sentence, then stop typing and do nothing else (don't click away) | Within ~5 s the teacher's Monitor / results page shows the answer and marks the item answered, with no `change` event on the client (no click-away) | ✅ 2026-09-17 — Debug build from `d4f9e13`, simulated lockdown, origin rev 38: typed in the essay with no click-away; the Monitor row read In progress · 1 of 10 answered on the next poll (predictive text visible under the simulator, as expected) |
| **The 30 s ceiling saves a non-stop typist.** Type continuously into an essay for a full minute without pausing 5 s and without leaving the field | At least one save lands on the teacher's side partway through the minute (the ceiling firing), not just at the end | ✅ 2026-09-21 — Debug build from `4e2da9f` (v1.3.4 + H-1), simulated lockdown, origin rev 46, sitting on `Client rows hand-run 2026-09-08 (copy)`: 60 s of continuous typing → three essay posts ~30 s apart (16:11:2x / 16:11:5x / 16:12:2x PT) in stderr; the ceiling fired |
| **Force-quit mid-essay loses at most the ceiling window (Debug).** Type into an essay, wait for at least one autosave to land, keep typing for under 30 s, then force-quit with `SECURE_TEST_DEBUG_CRASH=1` (Session menu "Trigger Debug Crash") | On the next launch, Resume shows the text up to the last autosave — at most ~30 s of the most recent typing is missing, nothing older | ✅ 2026-09-21 — autosave landed (essay post count 7→8), ~15 s more typing, Session → Trigger Debug Crash (SIGABRT line in stderr, no further post); relaunch + Resume showed the text up to the autosave, the last sentence ("row seven…") missing; the crash line drained on the next sign-in |
| **Word counter and answered mark are unchanged.** Type into an essay and short-text field, letting autosave fire | The word counter updates on every keystroke as before; the answered mark behaves exactly as it does on a `change` post today (non-empty text marks answered, no new rule) | ✅ 2026-09-21 — counter moved per keystroke; the Monitor read 1 of 10 answered after the first essay save |
| **An unchanged field posts nothing.** Click into an essay that already has saved text, then click out without typing anything (or type and delete back to the exact same text) and wait past 5 s | No new POST for that item — the teacher's "last activity" timestamp does not move and the server log shows nothing for it | ✅ 2026-09-21 — isolated click-in / click-out with no typing, 10 s: essay post count stayed at 5. Reading **AS-1**: after an autosave, LEAVING the field posts the same text once more (`change` fires because the value differs from focus-time; the handler posts unconditionally) — seen on essay, short_text and table; one extra post per focus, no data effect; proposal = a `current() === lastPosted` guard in the change handler |
| **short_text autosaves and keeps its math preview.** Type a `$…$` short-text answer and pause 5 s without leaving the field | The formula preview renders as it does today, and the answer autosaves the same as essay | ✅ 2026-09-21 — `$…$` answer, 5 s pause: preview rendered, one short_text post |
| **A table cell autosaves independently.** Open a table item, type into one cell, pause 5 s without leaving the cell or the grid | That cell's value reaches the server (whole-grid post as `change` does); a second cell left untouched does not post | ✅ 2026-09-21 — one cell, 5 s pause in the cell: one table post (the grid); the second post came only on leaving the item (AS-1) |
| **The E12 inline outline autosaves.** On a per-student stimulus with the inline outline field, type a line and pause 5 s | The outline text autosaves the same as essay, and a later `change` (leaving the field) still posts once with no duplicate | NOT EXERCISABLE 2026-09-21 — the fixture has no per-student stimulus; same code path as essay (`textAutosave`) |
| **A Cmd-Tab-away field still saves on page turn.** Type into an essay on a paged assessment, Cmd-Tab to another app before the 5 s timer fires (leaving the field unblurred), then Cmd-Tab back and turn the page (Next) | The dirty text is flushed and saved when the page turns, even though the field never lost focus in the browser sense | ✅ 2026-09-21 — typed, Cmd-Tab away within 5 s, back, Next: the essay post landed on the page turn |
| **Close mid-essay while offline defers the write (real AAC session).** Turn the Mac's network OFF, then have the teacher press Close session while the student is mid-essay | The client's own "Your teacher ended the test session." sheet appears (from the poll once network returns, or the write's own 409); the student's most recent unsent words are NOT yet on the teacher's page | ✅ 2026-09-21 — REAL AAC session, Mac on the phone's hotspot (the Wi-Fi menu is suppressed in-session; the first try failed because the Mac auto-rejoined the room's Wi-Fi — turn Auto-Join OFF for every other saved network first): hotspot off → typed "deferred words" → Close from the phone → hotspot on → the sheet appeared; stderr `1 answer(s) deferred until the next session`, the spool row carried the text with `deferred_at` set; the teacher's page did NOT have the words. Observation: the peek poll reported closed on the tick after the student pressed Esc — timing of the hotspot rejoin, not Esc |
| **A deferred write lands on the next Resume (real AAC session).** Continuing from the row above: open a new session for the same student and Resume | The essay shows the deferred text restored, and it now appears on the teacher's page (the deferred flush ran before the page rendered) | ✅ 2026-09-21 — new sitting, Resume: the essay showed "deferred words", stderr `deferred spool: 1 sent, 0 dropped, 0 still deferred`, spool empty, the teacher's per-student page carried the words |
| **A hand-in before Resume wins over the deferred row.** Repeat the offline-Close scenario (simulated lockdown is fine here), then have the teacher Hand in the attempt from the Monitor BEFORE the student resumes; then let the student resume | The resume drops the deferred row silently (no error, no sheet about lost text); the teacher's already-handed-in copy stands unchanged | ✅ 2026-09-21 — simulated lockdown: offline Close deferred the row; Hand in from the Monitor (session closed); new sitting → the list row read Done and the code field gave the H-1 wording; no error about lost text; the handed-in essay had neither "offline words" nor the crash-cut sentence. Reading **AS-2**: the deferred row stayed in the spool (the refusal returns before the bundle fetch) and was dropped on the next join (`deferred spool: 0 sent, 1 dropped`) — inert, no user effect |
| **T-3 verify-only: the 1-minute notice after the banner is hidden (Terminal launch).** On `Time limit hand-run 2026-09-11` (3-minute limit), hide the countdown banner with its × as soon as it appears, then wait; watch stderr | `time limit: 1 minute left` in stderr and the 1-minute notice on screen about two minutes in, banner hidden or not (the notice is host-driven, independent of the strip). If it does NOT fire, T-3 becomes a build item | ✅ 2026-09-21 — `FBUGQ7` then a second sitting `MJS2V3` on the 3-minute fixture, banner hidden at once both times: stderr `time limit: banner hidden by the student` → `time limit: 1 minute left` → `time limit reached — ending the secure session`, the session ended at zero; on the second run James watched the screen and the 1-minute notice appeared with the banner hidden. T-3 CLOSED — verify-only, not a build item |
| **Drawing toolbar is one Tab stop (4b-f).** On a drawing item, Tab from the stem to the toolbar, then Right / Left / Home / End; switch to Eraser and press Right through the colours | Tab lands on Pen only; arrows move focus along the strip and wrap; with Eraser on, the disabled colour buttons are skipped; Undo is skipped until a stroke exists; a further Tab leaves the strip for the canvas | ✅ 2026-09-21 — James: Tab lands on Pen, arrows / Home / End move and wrap, Eraser skips the colours, Tab leaves for the canvas |
...
| Check | Expect | Result |
|---|---|---|
| **Version.** About Secure Test | Reads 1.3.5 and the build commit | ✅ 2026-09-22 — "Version 1.3.5 (5b5bf41e0f82)" |
| **Close while locked, slowest Mac available (real AAC, D-1).** Join, answer one question, teacher presses Close session | The "Your teacher ended the test session." sheet; stderr shows `DID END` and never `lockdown did not confirm within 20s`; if the end took over 10 s, `still waiting for DID END after 10s (grace 20s)` precedes `DID END`; the app does NOT exit 70 | ✅ 2026-09-24 — Release 1.3.5 (26f482406fd8) from `~/Applications`, real AAC session, Keyboard navigation OFF: the "Your teacher ended the test session." sheet appeared (James); teacher Close → `peek: sitting reported closed by the server` → `lockdown ending: sitting_closed` → `DID END`; no `still waiting` / `did not confirm` line (under 10 s); no exit 70 |
| **Every exit still ends a real session (real AAC, AAC exit-path rule).** Three joins: Cmd-E; the titlebar exit; Cmd-Q | Cmd-E and the titlebar return to Your tests with the session ended (`DID END`); Cmd-Q quits with exit 0; none waits more than 20 s | ✅ 2026-09-24 — Release 1.3.5 (26f482406fd8) from `~/Applications`, real AAC session, Keyboard navigation OFF: five Cmd-E / titlebar ends (`emergency end control pressed` → `DID END`, none reached the 10 s line — the log does not tell the two controls apart; James ran both) and Cmd-Q (`quit requested with lockdown active` → `DID END`, the app quit; exit code not captured — the launcher detaches the app) |
| **Time is up still ends at zero (real AAC).** `Time limit hand-run 2026-09-11` (3-minute limit), let it run out | "Time is up." sheet, `DID END`, back to Your tests; no unrecoverable line | ✅ 2026-09-24 — Release 1.3.5 (26f482406fd8) from `~/Applications`, real AAC session, Keyboard navigation OFF: after the EX-1 earlier change: `5 minutes left` → `1 minute left` → `time limit reached` → `time_expired` → `DID END`; no unrecoverable line. Run on an Extend-timed attempt rather than the 3-minute fixture (same path) |
...
| **Cancelled sign-in keeps its words (D-3).** Sign in, press Cancel in the sheet | The card reads "Sign-in was cancelled." and keeps it after the refresh (it used to snap back to the bare "Sign in with your school Google account first.") | ✅ 2026-09-22 — 1.3.5 Debug, simulated lockdown: "Sign-in was cancelled." still on the card after the refresh; stderr `signin_failed: cancelled`. Re-run on `19a4d8b` (sign-in log trim): unchanged |
| **Failed sign-in keeps its words (D-3).** Network off, press Sign in | "Could not sign in…" (or the presenter's failure line) stays on the card after the refresh | ✅ 2026-09-22 — 1.3.5 Debug (`5b26889`), Wi-Fi off, James at the keyboard: the sheet closed at once, "Could not sign in. Tell your teacher." appeared and stayed through the refresh; stderr `signin_failed` NSURLErrorDomain -1009 ("The Internet connection appears to be offline."). Reading: that line carried the full authorize URL (client id, single-use state / nonce / PKCE challenge) into `errors.log` and the drained `client_error_events` row — trimmed in `19a4d8b` (`signInFailureSummary`). Re-run on `19a4d8b` the same evening: same card behaviour; stderr + `signin_failed` now read `NSURLErrorDomain -1009: The Internet connection appears to be offline.` with no URL |
| **A superseded navigation no longer ends sign-in (D-3, -999).** Sign in through the full Google → ClassLink → MFA chain, first sign-in of the day if possible (SI-1's shape) | The sheet stays up through every redirect and completes; stderr has no `signin_failed` carrying `-999`; no silent return to the card | ✅ 2026-09-22 — 1.3.5 Debug, simulated lockdown: six sign-ins through the full chain (prompt=login), staff and a demo student, every one minted; no `-999` anywhere in stderr |
| **Leaving a field after an autosave posts nothing more (D-4, AS-1).** Type in an essay, wait 5 s for the autosave, then click out without typing | One post only (stderr / server log); the Monitor's last activity does not move on the click-out. Type one more character and click out → one post | ✅ 2026-09-22 — Debug build from `5b5bf41` (1.3.5), simulated lockdown, origin rev 48, practice sitting on `Math entry + drawing tools hand-run 2026-09-22`, Claude driving the client (computer use) + Chrome: short_text — autosave + click-out = 1 post, one more character + click-out = 2; essay — autosave + click-out, then "." + click-out = 2 posts |
| **Practice row label (practice slice 3).** Teacher opens Practice on my Mac; sign in to the client as that teacher | Your tests lists the row labelled "Practice — only you"; joining starts the attempt (simulated lockdown here; real on the signed build) | ✅ 2026-09-22 — 1.3.5 Debug, simulated lockdown: "K5T6YB · Practice — only you — <teacher>" |
| **Staff with nothing to practise.** Sign in as staff with no open practice sitting | The list reads "No practice tests right now. Start one from your assessment's Test sessions tab." — not the student `not_on_roster` copy | ✅ 2026-09-22 — 1.3.5 Debug, simulated lockdown: "No practice tests right now. Start one from your assessment's Test sessions tab." |
| **Staff wrong code (follow-up 5cf4639).** Signed in as staff, type a class sitting's code, then a closed code | Both read "That code is not open for you. A practice test opens only for the teacher who started it." | ✅ 2026-09-22 — 1.3.5 Debug, simulated lockdown: an open class sitting's code and a closed code both read "That code is not open for you. A practice test opens only for the teacher who started it." (server 404 `session_unavailable`) |
| **Student wrong code unchanged.** Signed in as a demo student, type a closed code | "That code is not open for you right now. Check it with your teacher." | ✅ 2026-09-22 — 1.3.5 Debug, simulated lockdown, demo student: a closed code read "That code is not open for you right now. Check it with your teacher." |
...
| Check | Expect | Result |
|---|---|---|
| **EX-1 later.** Timed attempt in progress; the teacher presses Extend time on the Monitor row and picks a LATER time | Within ~5 s: the notice "Your teacher changed your time. The test now ends at <time>." and the countdown jumps to the new deadline; stderr `time limit: deadline changed by the teacher — Ns left` | ✅ 2026-09-24 — Release 1.3.5 (26f482406fd8) from `~/Applications`, real AAC session, Keyboard navigation OFF: `deadline changed by the teacher — 903s left` within one poll; notice + countdown jumped |
| **EX-1 earlier.** Same, a time a few minutes from now (under 5 min) | The countdown drops; the 5-minute notice (and the 1-minute one when due) follows the change notice; at zero the session ends with "Time is up." | ✅ 2026-09-24 — Release 1.3.5 (26f482406fd8) from `~/Applications`, real AAC session, Keyboard navigation OFF: `— 83s left`, then the 5- and 1-minute notices, then "Time is up." at zero |
| **EX-1 untimed → timed.** An untimed test in progress; the teacher extends it to a time 10 min from now | A "Time left" strip appears at the top of the page where a timed test shows it, counting to the new time, plus the change notice | ✅ 2026-09-24 — Release 1.3.5 (26f482406fd8) from `~/Applications`, real AAC session, Keyboard navigation OFF: untimed attempt (no `time limit:` line at join) → Extend → `— 494s left`; the strip appeared with the notice |
| **EX-1 hidden timer.** Hide the timer (×), then the teacher extends | The strip stays hidden; the change notice still shows | ✅ 2026-09-24 — Release 1.3.5 (26f482406fd8) from `~/Applications`, real AAC session, Keyboard navigation OFF: `banner hidden by the student` → `— 608s left`; strip stayed hidden, notice shown |
| **EX-1 no churn.** Timed test, no teacher action, sit for 1 minute | No change notice, no `deadline changed` line (poll jitter is not a change) | ✅ 2026-09-24 — Release 1.3.5 (26f482406fd8) from `~/Applications`, real AAC session, Keyboard navigation OFF: a minute with no teacher action: no notice, no `deadline changed` line |
| **Keyboard reach — choices.** Single-choice question: click the stem area, press Tab | Focus lands on ONE choice (the selected one, else the first) with a visible ring; ↓ / → select the next choice and ↑ / ← the previous, wrapping; each move saves (the teacher side shows the new answer); Tab leaves the group | ✅ 2026-09-24 — Release 1.3.5 (26f482406fd8) from `~/Applications`, real AAC session, Keyboard navigation OFF: one stop, arrows select and wrap, 13 saves logged for 13 moves |
| **Keyboard reach — multi, order, match.** Multi-select, order (Move buttons), match | Tab reaches every checkbox, every Move button and every dropdown, each with a visible ring (the dropdowns had none before) | ✅ 2026-09-24 — Release 1.3.5 (26f482406fd8) from `~/Applications`, real AAC session, Keyboard navigation OFF: every checkbox, Move button and dropdown reached with a ring |
| **Keyboard reach — hotspot (HS-1).** Tab through a hotspot question | Each region is its own Tab stop with the thick ring; Space toggles it | ✅ 2026-09-24 — Release 1.3.5 (26f482406fd8) from `~/Applications`, real AAC session, Keyboard navigation OFF: each region its own stop, Space toggles |
| **Keyboard reach — Math keys (ME-1).** Short-text `$` item: Tab from the field, Tab again, then Shift-Tab twice | Field → the "Math keys" toggle → Fraction; Shift-Tab: Fraction → toggle → field | ✅ 2026-09-24 — Release 1.3.5 (26f482406fd8) from `~/Applications`, real AAC session, Keyboard navigation OFF: field → toggle → Fraction and back |
| **Keyboard reach — page controls.** Paged test: keep tabbing past the last question control | Previous / Next, the page-strip buttons, Clear answer, and on the review page Finish, each reachable with a ring | ✅ 2026-09-24 — Release 1.3.5 (26f482406fd8) from `~/Applications`, real AAC session, Keyboard navigation OFF: Previous / Next, strip, Clear answer, Finish reached |
| **Keyboard reach — added later.** After a page turn, after the time strip appears (EX-1 untimed row), and in the picture overlay | The new controls (the strip's ×, the overlay's close) are reachable by Tab — the MutationObserver path, not testable headlessly | ✅ 2026-09-24 — Release 1.3.5 (26f482406fd8) from `~/Applications`, real AAC session, Keyboard navigation OFF: after a page turn, the strip's × (closed from the keyboard) and the overlay close |
| **Keyboard reach — roving groups unchanged.** Drawing toolbar and math keypad | Still ONE Tab stop each, arrows move within | ✅ 2026-09-24 — Release 1.3.5 (26f482406fd8) from `~/Applications`, real AAC session, Keyboard navigation OFF: drawing toolbar and keypad still one stop each |
| **ME-3 preview.** VoiceOver on; type `\frac{1}{2}` in a short-text field | The preview is announced as a fraction ("1 over 2" / "one half"), never the literal `\frac{1}{2}`. If it goes silent, record that — the fallback is a visually hidden spoken-text span | ✅ 2026-09-24 — Release 1.3.5 (26f482406fd8) from `~/Applications`, real AAC session, Keyboard navigation OFF: VoiceOver on before joining; the VO cursor (Control-Option-Right from the field) lands on the preview and reads the fraction correctly. The field itself is spelled letter by letter (it holds the literal text) and typing echo speaks each key — both expected. The preview is not a Tab stop and not mouse-selectable (the page disables selection outside inputs) — by design |
| **ME-3 stem.** VoiceOver on a stem carrying `$\frac{1}{4}$` | Read as math, not LaTeX | ✅ 2026-09-24 — Release 1.3.5 (26f482406fd8) from `~/Applications`, real AAC session, Keyboard navigation OFF: VO cursor on the `$\frac{1}{4}$` stem reads it as math |

Reading **EX-2** (2026-09-24, proposal only): joining an attempt whose deadline had
already passed (`time limit: -81226s left`) still called `begin()`; the end was
deferred until `DID BEGIN` (finding 8.3's path), `page load gate: opened` logged
...
| Check | Expect | Result |
|---|---|---|
| **Removed for one student.** Timed attempt in progress with a few minutes left; on the Monitor tick that student, Adjust time for selected → No time limit | Within ~5 s: the "Time left" strip leaves the page, the notice "Your teacher removed the time limit." shows (and takes itself down after 8 s); stderr `time limit: removed by the teacher` ONCE (later polls log nothing); the session is NOT ended when the old deadline passes — keep working past it and save an answer | ✅ 2026-09-24 (Debug build of `89f989c`, simulated lockdown, windowed, local dev; Claude drove the client + the Monitor in Chrome) — a 3-minute override, then Monitor tick → Adjust time for selected → No time limit: the strip left the page and "Your teacher removed the time limit." showed within the poll interval; stderr `time limit: removed by the teacher` once; the old deadline (+30 s grace) passed with the session up and an answer saved at 16:10:05, after it. Reading: computer-use mouse clicks did not select a radio in the web view (keyboard Tab + Space did) — a tooling artefact, not the app |
| **Removed for the whole session.** Two students in one timed sitting; Adjust time from the header → No time limit | Both clients drop the strip and show the notice; neither ends at the old zero. A third student joining afterwards gets no strip at all | ✅ 2026-09-24 — one client on screen (a second instance of the same bundle can't run beside it), the second student through the API: header Adjust time → No time limit dropped the on-screen strip ~1 s after the press with the notice; the second student's peek poll read `time_limit_removed: true`; a third student joining afterwards inherited the removal and got a bundle with no `time_limit_ends_at`. "Neither ends at the old zero" rests on row 1 (the old deadline here was 20 min out) |
| **Set again after removal.** Continue from the first row: Adjust time for that student → a time 10 min from now | The strip comes back at the top of the page counting to the new time, with the change notice "Your teacher changed your time…"; stderr `deadline changed by the teacher — Ns left`; the 5- and 1-minute notices and "Time is up." follow as usual | ✅ 2026-09-24 — continuing from row 1 with a 3-minute deadline (not 10, to reach zero quickly): the strip came back at the top ("Time left 2:58"), stderr `deadline changed by the teacher — 179s left`, then the 1-minute notice and "Time is up." at the new deadline (`lockdown DID END`, sheet "Time is up. Your answers are saved."). Reading: with under 5 minutes left the "5 minutes left" notice replaced the change notice in the same second; the change notice itself ("Your teacher changed your time. The test now ends at 4:25 PM.") was seen on the row-4 run with 10 minutes |
