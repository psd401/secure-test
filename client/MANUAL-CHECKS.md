...
| Check | Expect | Result |
|---|---|---|
| **Close ends the student's session (real session).** Student joined and typing in an essay; teacher presses Close session on the Monitor | Within ~5 s the Mac unlocks, the test comes off the screen, "Your tests" is back, and a one-button sheet reads **"Your teacher ended the test session."** / "Your answers are saved." stderr: `sitting closed (peek) — ending the secure session`, then `DID END` | ✅ 2026-09-15 (SIMULATED lockdown, Debug build from `f2cc2e5`, origin rev 34): Close 23:22:29.5Z → `lockdown_end` + `sitting_closed {via: peek}` 23:22:29.9Z; sheet copy exact; Your tests showed "No tests assigned right now". **Real-session half OPEN** — needs the student device (this Mac cannot lock while Claude drives Chrome: remote + DNS) |
...
| **A student mid-typing loses at most the unflushed field (D-3).** Student typing in an essay when Close lands; compare what the teacher's queue shows | Everything posted before Close is there; at most the field still holding focus is short of its last keystrokes — an autosave in flight at the moment of Close can be refused. No error dialog about lost answers | ✅ 2026-09-15 — essay "Here I am writing." (posted on leaving the field earlier) intact on the teacher page; no error dialog; no Needs-attention from the close |
...
| **Hand in is still the teacher's.** After the student is home, teacher presses Hand in on the Monitor row | The attempt finalises with the answers as they stood; the student's screen is unaffected (they are already on "Your tests") | NOT RUN — attempt kept in progress for the real-session run on the student device |
