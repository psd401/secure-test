...
| **The 30 s ceiling saves a non-stop typist.** Type continuously into an essay for a full minute without pausing 5 s and without leaving the field | At least one save lands on the teacher's side partway through the minute (the ceiling firing), not just at the end | ⚠️ 2026-09-17 — saves landed at 15:55:41 / 15:56:35 / 15:56:45 / 15:57:02Z during the 3-minute essay (several mid-typing), but the 54 s gap means a pause happened; the ceiling itself is proven by the unit test only — re-run with strictly continuous typing |
